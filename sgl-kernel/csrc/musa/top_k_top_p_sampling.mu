...
void musa_min_p_sampling_from_probs(
    at::Tensor probs,
    at::Tensor output,
    std::optional<at::Tensor> maybe_indices,
  ...
  const c10::musa::OptionalMUSAGuard device_guard(device);
  auto stream = at::musa::getCurrentMUSAStream();
  musaError_t status = flashinfer::sampling::MinPSamplingFromProb<float, int>(
      static_cast<float*>(probs.data_ptr()),
      maybe_min_p_arr.has_value() ? static_cast<float*>(maybe_min_p_arr->data_ptr()) : nullptr,
      static_cast<int*>(output.data_ptr()),
      maybe_indices.has_value() ? static_cast<int*>(maybe_indices->data_ptr()) : nullptr,
      batch_size,
      min_p_val,
  ...
  TORCH_CHECK(
      status == musaSuccess,
      "MinPSamplingFromProb failed with error code " + std::string(musaGetErrorString(status)));
}
