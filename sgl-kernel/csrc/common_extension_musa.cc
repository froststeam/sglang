...
TORCH_LIBRARY_EXPAND(sgl_kernel, m) {
  ...
  m.def(
      "musa_min_p_sampling_from_probs(Tensor probs, Tensor output, Tensor? maybe_indices, Tensor? "
      "maybe_min_p_arr, float min_p_val, bool deterministic, Generator? gen) -> ()");
  m.impl("musa_min_p_sampling_from_probs", torch::kMUSA, &musa_min_p_sampling_from_probs);
