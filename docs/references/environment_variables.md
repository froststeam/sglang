...
| `SGLANG_MUSA_USE_MULTI_STREAM` | Allocate alt stream on MUSA to overlap shared and routed experts in DeepseekV2 MoE. Best paired with `--deepep-mode low_latency` so Mori's AsyncLL kernel offloads dispatch/combine to copy engines and frees CUs. | `false` |
| `SGLANG_MUSA_DISABLE_OVERLAP_SCHEDULE` | Whether MUSA force-disables the overlap scheduler. Default `true` (MUSA disables overlap by default). Set `false`/`0` to keep the overlap scheduler enabled on MUSA where it is correct and beneficial. Speculative decoding / pipeline parallelism still disable overlap regardless. | `true` |
