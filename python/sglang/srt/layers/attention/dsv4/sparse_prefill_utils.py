...
def combine_topk_swa_indices(
    ...
    Args:
        ...
        query_start_loc: (num_reqs+1,) int32. Cumulative query lengths; may
            ...
            ``query_start_loc[0]``.
        seq_lens: (num_reqs,) int32. Each local request's last visible KV
            length. This equals the full sequence length without CP, and the
            current rank's last query position + 1 under round-robin CP.
        seq_lens_casual: (num_tokens,) int32. Per-query visible KV length,
            matching ``DeepseekV4AttnBackend.make_core_attn_metadata``. Under
            round-robin CP, these are not a contiguous suffix of each request,
            so the combine kernel must read them directly instead of deriving
            positions from ``seq_lens`` and ``query_start_loc``.
    ...
    assert seq_lens_casual.shape == (num_tokens,)
...
def build_swa_token_ids(
    ...
    Per request, the union spans from the first local query's SWA-window start
    to the last local query position. Without CP this is the usual
    ``[max(0, seq_len - extend - W + 1), seq_len)`` range. Under round-robin
    CP, local queries are strided in global position space, so the range can
    be wider than ``extend + W - 1``. Each position is translated through
    ``req_to_token`` (full kv-cache id) and then ``full_to_swa`` (SWA cache
    id) to land in the SWA-cache token-id space that
    ``dequantize_k_cache_paged`` consumes.

    Args:
        seq_lens: (num_reqs,) int32, per-local-request last visible KV length.
        seq_lens_casual: (num_tokens,) int32, per-query visible KV length.
    ...
    if seq_lens_casual.numel() == 0:
        ...
        first_q_per_req = query_start_loc[:-1].long()
        first_visible_lens = seq_lens_casual[first_q_per_req]
        swa_first_pos = torch.clamp(first_visible_lens - swa_window, min=0).to(
            torch.int32
        )
        swa_gather_lens = (seq_lens - swa_first_pos).clamp_min(0).to(torch.int32)
...
def _combine_topk_swa_indices_kernel(
    ...
    for token_idx in range(query_start + worker_id, query_end, num_workers):
        pos = tl.load(seq_lens_casual_ptr + token_idx) - 1
...
class SparsePrefillChunkCache:
    ...
    swa_page_size: int
    seq_lens: torch.Tensor  # (num_reqs,) int32, last visible KV length
    seq_lens_casual: torch.Tensor  # (num_qo_tokens,) int32
    ...
    def build(
        ...
        device = seq_lens.device
        num_reqs = seq_lens.shape[0]
        assert seq_lens.shape == extend_seq_lens.shape == req_pool_indices.shape
        assert seq_lens_casual.shape == (num_qo_tokens,)

        query_start_loc = torch.zeros(num_reqs + 1, dtype=torch.int32, device=device)
        query_start_loc[1:] = torch.cumsum(extend_seq_lens, dim=0).to(torch.int32)
        assert int(query_start_loc[-1].item()) == num_qo_tokens, (
            f"sparse prefill cache local q length mismatch: "
            f"{int(query_start_loc[-1].item())=} {num_qo_tokens=}"
        )
    ...
    def combine_c4_layer(
        ...
        """Per-layer combine for c4 using request-local raw topk indices."""
