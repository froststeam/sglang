...
class MoriEPDispatcher(BaseDispatcher):
    def __init__(
        ...
        multi_stream_env = next(
            (
                env_name
                for env_name in (
                    "SGLANG_ROCM_USE_MULTI_STREAM",
                    "SGLANG_MUSA_USE_MULTI_STREAM",
                )
                if get_bool_env_var(env_name)
            ),
            None,
        )
        if multi_stream_env is not None and not async_mode:
            logger.warning_once(
                "%s=1 is set but Mori AsyncLL is "
