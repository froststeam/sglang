...
import torch
from flash_attn import flash_attn_varlen_func as _mate_flash_attn_varlen_func
    ...
    if ctx.use_mla:
        if _MATE_MLA_WORKSPACE_BUFFER is None:
            _MATE_MLA_WORKSPACE_BUFFER = torch.empty(
                _MATE_MLA_WORKSPACE_SIZE_BYTES, device=ctx.device, dtype=torch.uint8
            )
        ...
        with _MATE_NO_MLA_SCHEDULER_METADATA_LOCK:
            if should_update or ctx.prefix not in _MATE_NO_MLA_SCHEDULER_METADATA_DICT:
                _MATE_NO_MLA_SCHEDULER_METADATA_DICT[ctx.prefix] = (
