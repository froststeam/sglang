...
void sgl_musa_rmsnorm(ffi::TensorView input, ffi::TensorView weight,
  ...
  if (dtype_equal(input.dtype(), dl_float16)) {
    ...
    if (gemma) {
      launch_rmsnorm<__mt_bfloat16, true>(input, weight, out,
                                          static_cast<float>(eps));
    } else {
      launch_rmsnorm<__mt_bfloat16, false>(input, weight, out,
                                           static_cast<float>(eps));
    }
  } else {
  ...
  TVM_FFI_ICHECK_EQ(err, musaSuccess)
      << "MUSA rmsnorm kernel failed: " << musaGetErrorString(err);
}

void sgl_musa_fused_add_rmsnorm(ffi::TensorView input,
                                ffi::TensorView residual,
                                ffi::TensorView weight, double eps,
                                bool gemma) {
  check_fused_inputs(input, residual, weight);
  ffi::MUSADeviceGuard device_guard(input.device().device_id);
  if (dtype_equal(input.dtype(), dl_float16)) {
    if (gemma) {
      launch_fused_add_rmsnorm<half, true>(input, residual, weight,
                                           static_cast<float>(eps));
    } else {
      launch_fused_add_rmsnorm<half, false>(input, residual, weight,
                                            static_cast<float>(eps));
    }
  } else if (dtype_equal(input.dtype(), dl_bfloat16)) {
    if (gemma) {
      launch_fused_add_rmsnorm<__mt_bfloat16, true>(
          input, residual, weight, static_cast<float>(eps));
    } else {
      launch_fused_add_rmsnorm<__mt_bfloat16, false>(
          input, residual, weight, static_cast<float>(eps));
    }
  } else {
