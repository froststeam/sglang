...
#define CAL_MOE_GEMV_INT4(_ADTYPE, _BDTYPE, _TOPK_WEIGHT_DTYPE, _SCALE_DTYPE, _IS_MUL_ROUTED_WEIGHT, _IS_SWGELU) \
    if (is_pergroup_scale) { \
        if (scale_k_group_tile == 128) { \
            musa_moe_gemv_generic_kernel<_ADTYPE, _BDTYPE, _ADTYPE, _TOPK_WEIGHT_DTYPE, _SCALE_DTYPE, block_n, block_k, iobit, _IS_MUL_ROUTED_WEIGHT, _IS_SWGELU, false, true, true, false, 128, false, check_bounds> \
            ...
            musa_moe_gemv_generic_kernel<_ADTYPE, _BDTYPE, _ADTYPE, _TOPK_WEIGHT_DTYPE, _SCALE_DTYPE, block_n, block_k, iobit, _IS_MUL_ROUTED_WEIGHT, _IS_SWGELU, false, true, true, false, 64, false, check_bounds> \
                <<<grid_size, block_size, shmem_size, stream>>>( \
                    static_cast<_ADTYPE*>(C.data_ptr()), \
                    static_cast<_ADTYPE*>(A.data_ptr()), \
                    static_cast<_BDTYPE*>(B.data_ptr()), \
        ...
        musa_moe_gemv_generic_kernel<_ADTYPE, _BDTYPE, _ADTYPE, _TOPK_WEIGHT_DTYPE, _SCALE_DTYPE, block_n, block_k, iobit, _IS_MUL_ROUTED_WEIGHT, _IS_SWGELU, false, true, false, false, 1, false, check_bounds> \
            <<<grid_size, block_size, shmem_size, stream>>>( \
                static_cast<_ADTYPE*>(C.data_ptr()), \
                static_cast<_ADTYPE*>(A.data_ptr()), \
                static_cast<_BDTYPE*>(B.data_ptr()), \
...
#define GEN_LAUNCH_INT4(_BLK_N, _BLK_K, _CHECK_BOUNDS) \
    { \
        launch_kernel = [&]() { \
            ...
            TORCH_CHECK(false, "unsupported W4A16 MoE GEMV dtype"); \
        }; \
    }
...
void launch_moe_gemv_w4a16(
    ...
    TORCH_CHECK(config_id == -1, "W4A16 MoE GEMV does not expose tunable configs");
    ...
    TORCH_CHECK(has_b_scale, "B_scale is required for W4A16 MoE GEMV");
    ...
    TVM_FFI_ICHECK_EQ(err, musaSuccess)
        << "MUSA MoE INT4 GEMV kernel failed: " << musaGetErrorString(err);
}
...
void sgl_musa_moe_gemv(
    ...
    bool use_swigelu,
    int64_t config_id) {
    TORCH_CHECK(!has_a_scale, "W4A16 MoE GEMV does not use A_scale");
    TORCH_CHECK(use_w4a16, "W4A16 MoE GEMV requires the W4A16 path");
    launch_moe_gemv_w4a16(A, B, C, B_scale, topk_weights, topk_ids,
                         has_b_scale, mul_routed_weight, topk, use_swigelu,
                         config_id);
}
