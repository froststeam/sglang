...
template <int SUBWARPS_PER_BLOCK, int ACTIVATION_TYPE>
...
void launch_act_and_mul_masked_post_quant(
  ...
  if (subwarps_per_block == 8) {
    act_and_mul_masked_post_quant_kernel<8, ACTIVATION_TYPE><<<grid, block, 0, stream>>>(
    ...
    act_and_mul_masked_post_quant_kernel<4, ACTIVATION_TYPE><<<grid, block, 0, stream>>>(
        static_cast<const __mt_bfloat16*>(input.data_ptr()),
        static_cast<__mt_fp8_e4m3*>(output.data_ptr()),
        static_cast<float*>(output_scale.data_ptr()),
        static_cast<const int32_t*>(masked_m.data_ptr()),
    ...
    act_and_mul_masked_post_quant_kernel<2, ACTIVATION_TYPE><<<grid, block, 0, stream>>>(
        static_cast<const __mt_bfloat16*>(input.data_ptr()),
        static_cast<__mt_fp8_e4m3*>(output.data_ptr()),
        static_cast<float*>(output_scale.data_ptr()),
        static_cast<const int32_t*>(masked_m.data_ptr()),
    ...
    act_and_mul_masked_post_quant_kernel<1, ACTIVATION_TYPE><<<grid, block, 0, stream>>>(
        static_cast<const __mt_bfloat16*>(input.data_ptr()),
        static_cast<__mt_fp8_e4m3*>(output.data_ptr()),
        static_cast<float*>(output_scale.data_ptr()),
        static_cast<const int32_t*>(masked_m.data_ptr()),
...
void sgl_musa_act_and_mul_masked_post_quant(
  ...
  if (act == kGeluActivation) {
    launch_act_and_mul_masked_post_quant<kGeluActivation>(input, output, output_scale, masked_m, stream);
  } else if (act == kGeluTanhActivation) {
    launch_act_and_mul_masked_post_quant<kGeluTanhActivation>(input, output, output_scale, masked_m, stream);
  } else {
    launch_act_and_mul_masked_post_quant<kSiluActivation>(input, output, output_scale, masked_m, stream);
  }
