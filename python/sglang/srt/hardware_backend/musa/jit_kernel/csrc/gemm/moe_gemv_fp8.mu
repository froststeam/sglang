...
#define CAL_MOE_GEMV_FP8(_ADTYPE, _BDTYPE, _CDTYPE, _IS_MUL_ROUTED_WEIGHT, _IS_SWGELU) \
    if (scale_k_group_tile == 128) { \
        musa_gemv_kernel<_ADTYPE, _BDTYPE, _CDTYPE, float, float, block_n, block_k, iobit, _IS_MUL_ROUTED_WEIGHT, _IS_SWGELU, false, false, true, true, 128, false, check_bounds> \
        ...
        musa_gemv_kernel<_ADTYPE, _BDTYPE, _CDTYPE, float, float, block_n, block_k, iobit, _IS_MUL_ROUTED_WEIGHT, _IS_SWGELU, false, false, true, true, 64, false, check_bounds> \
            <<<grid_size, block_size, shmem_size, stream>>>( \
                static_cast<_CDTYPE*>(C.data_ptr()), \
                static_cast<_ADTYPE*>(A.data_ptr()), \
                static_cast<_BDTYPE*>(B.data_ptr()), \
        ...
        musa_gemv_kernel<_ADTYPE, _BDTYPE, _CDTYPE, float, float, block_n, block_k, iobit, _IS_MUL_ROUTED_WEIGHT, _IS_SWGELU, false, false, false, true, 1, false, check_bounds> \
            <<<grid_size, block_size, shmem_size, stream>>>( \
                static_cast<_CDTYPE*>(C.data_ptr()), \
                static_cast<_ADTYPE*>(A.data_ptr()), \
                static_cast<_BDTYPE*>(B.data_ptr()), \
...
#define RUN_FP8_ROUTE(_ADTYPE, _BDTYPE, _CDTYPE) \
    if (mul_routed_weight) { \
        if (use_swigelu) { \
            CAL_MOE_GEMV_FP8(_ADTYPE, _BDTYPE, _CDTYPE, true, true) \
        } else { \
            CAL_MOE_GEMV_FP8(_ADTYPE, _BDTYPE, _CDTYPE, true, false) \
        } \
    } else { \
        if (use_swigelu) { \
            CAL_MOE_GEMV_FP8(_ADTYPE, _BDTYPE, _CDTYPE, false, true) \
        } else { \
            CAL_MOE_GEMV_FP8(_ADTYPE, _BDTYPE, _CDTYPE, false, false) \
        } \
    }
    ...
    { \
        launch_kernel = [&]() { \
            ...
            if (dtype_equal(A.dtype(), dl_bfloat16)) { \
                RUN_FP8_ROUTE(bfloat16_t, __mt_fp8_e4m3, bfloat16_t) \
            } else { \
                RUN_FP8_ROUTE(__mt_fp8_e4m3, __mt_fp8_e4m3, bfloat16_t) \
            } \
...
void launch_moe_gemv_fp8(
    ...
    TVM_FFI_ICHECK_EQ(err, musaSuccess)
        << "MUSA fused MoE FP8 GEMV kernel failed: " << musaGetErrorString(err);
}
...
void sgl_musa_fused_moe_gemv(
    ...
    bool use_int4_w4a16,
    bool use_swigelu) {
    TORCH_CHECK(!use_int4_w4a16, "fp8 moe gemv does not use int4 path");
    launch_moe_gemv_fp8(A, B, C, A_scale, B_scale, topk_weights, topk_ids, has_a_scale, has_b_scale, mul_routed_weight, topk, use_swigelu);
}
